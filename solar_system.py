...
# Buat merkurius
merkurius = sphere(pos=vector(5, 0, 0), radius=0.2, color=color.gray(0.5))
...
# Buat venus
venus = sphere(pos=vector(9, 0, 0), radius=0.3, color=color.yellow)

# nama untuk venus
venus_label = label(pos=venus.pos + vector(0, 1.5, 0), text="venus", height=15, box=False, color=color.white)

# Buat bumi
bumi = sphere(pos=vector(13, 0, 0), radius=0.5, color=color.blue)

# nama untuk bumi
bumi_label = label(pos=bumi.pos + vector(0, 1.5, 0), text="Earth", height=15, box=False, color=color.white)

# Buat mars
mars = sphere(pos=vector(17, 0, 0), radius=0.4, color=color.red)

# nama untuk mars
mars_label = label(pos=mars.pos + vector(0, 1.5, 0), text="Mars", height=15, box=False, color=color.white)

# Buat jupiter
jupiter = sphere(pos=vector(21, 0, 0), radius=1, color=color.orange)

# nama untuk jupiter
jupiter_label = label(pos=jupiter.pos + vector(0, 1.5, 0), text="jupiter", height=15, box=False, color=color.white)

# Buat saturnus
saturnus = sphere(pos=vector(25, 0, 0), radius=0.7, color=vector(1, 0.84, 0))

# Fungsi untuk membuat lapisan cincin saturnus
def create_ring(radius, thickness, color):
    return ring(pos=saturnus.pos, axis=vector(0, 1, 0), radius=radius, thickness=thickness, color=color)

# Buat lapisan cincin saturnus
rings = [
    create_ring(radius=1.2, thickness=0.05, color=vector(0.6, 0.6, 0.6)),  # Cincin dalam
    create_ring(radius=1.3, thickness=0.07, color=vector(0.8, 0.7, 0.5)),  # Cincin tengah
    create_ring(radius=1.5, thickness=0.1, color=vector(0.5, 0.5, 0.5)),   # Cincin luar
]
...
# Buat uranus
uranus = sphere(pos=vector(29, 0, 0), radius=0.6, color=color.cyan)

# nama untuk uranus
uranus_label = label(pos=uranus.pos + vector(0, 1.5, 0), text="uranus", height=15, box=False, color=color.white)

# Buat neptunus
neptunus = sphere(pos=vector(33, 0, 0), radius=0.4, color=color.blue)
...
while True:
    ...
    # Rotasi merkurius (lebih cepat)
    merkurius.pos = merkurius.pos.rotate(angle=0.02, axis=vector(0, 1, 0))

    # Rotasi venus (lebih cepat)
    venus.pos = venus.pos.rotate(angle=0.015, axis=vector(0, 1, 0))

    # Rotasi bumi (kecepatan sedang)
    bumi.pos = bumi.pos.rotate(angle=0.01, axis=vector(0, 1, 0))

    # Rotasi mars (kecepatan sedikit lebih lambat)
    mars.pos = mars.pos.rotate(angle=0.009, axis=vector(0, 1, 0))

    # Rotasi jupiter (lebih lambat)
    jupiter.pos = jupiter.pos.rotate(angle=0.005, axis=vector(0, 1, 0))

    # Rotasi saturnus (lebih lambat)
    saturnus.pos = saturnus.pos.rotate(angle=0.004, axis=vector(0, 1, 0))

    # Perbarui posisi cincin untuk mengikuti Saturnus
    for ring_layer in rings:
        ring_layer.pos = saturnus.pos

        # Rotasi cincin di sekitar Saturnus
        ring_layer.rotate(angle=0.02, axis=vector(0, 1, 0), origin=ring_layer.pos)

    # Rotasi uranus (lebih lambat)
    uranus.pos = uranus.pos.rotate(angle=0.003, axis=vector(0, 1, 0))

    # Rotasi neptunus (terlambat)
    neptunus.pos = neptunus.pos.rotate(angle=0.002, axis=vector(0, 1, 0))
